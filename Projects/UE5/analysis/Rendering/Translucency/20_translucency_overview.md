...
graph TD
    subgraph Light["照明準備（Render前）"]
        TV[RenderTranslucencyLightingVolume<br>3D テクスチャに照明を注入]
    end

    subgraph Render["RenderTranslucency()"]
        BT[Before DOF Pass<br>Separate Translucency 対象外]
        AT[After DOF Pass<br>Separate Translucency]
        OIT[OIT Accumulate<br>Weighted Blended OIT]
    end

    subgraph Composite["合成"]
        COMP[OIT Composite<br>加重平均でアルファブレンド]
        DIST[RenderDistortion<br>歪み効果]
    end
