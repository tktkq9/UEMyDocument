...
- 更新日: 2026-04-30（コード適合監査済 / 行番号を UE5 現行ソースに同期）

> **補足（2026-04-30 監査差分）**: 以下 4 箇所を UE5 現行ソースに合わせて修正済。
> - Lambert: `:175` → `:177`（コメント挿入で +2 行ズレ）
> - SmithJointApprox: `:392` → `:393`（+1）
> - Karis Split-Sum: `EnvBRDFApprox`（旧記載）→ `EnvBRDF`（612 行、PreIntegratedGF テクスチャ版）に**関数特定を訂正**。
>   旧記載の `EnvBRDFApprox` は内部で `EnvBRDFApproxLazarov` を呼ぶ Lazarov 多項式版（642 行）であり、Karis Split-Sum とは別アルゴリズム。
> - Lazarov 行に `:642 EnvBRDFApprox`（Lazarov を実用するラッパー）も併記。
...
| NDF | Anisotropic GGX | S03 | [[brdf_ggx_aniso]] | `BRDF.ush:339` `D_GGXaniso` | **完了 (2026-04-27)** |
| 幾何項 | Smith G2 (Joint Approx) | S04 | [[brdf_smith]] | `BRDF.ush:393` `Vis_SmithJointApprox` | **完了 (2026-04-26)** |
| 幾何項 | Schlick Visibility | S05 | [[brdf_schlick_vis]] | `BRDF.ush:374` `Vis_Schlick` | **完了 (2026-04-27)** |
| 幾何項 | Kelemen | S07 | [[brdf_kelemen]] | `BRDF.ush:366` `Vis_Kelemen` | **完了 (2026-04-27)** |
...
| 拡散 | Lambert | （古典） | [[brdf_lambert]] | `BRDF.ush:177` `Diffuse_Lambert` | **完了 (2026-04-27)** |
| IBL | Karis Split-Sum 近似 | S01 | [[brdf_split_sum]] | `BRDF.ush:612` `EnvBRDF`（PreIntegratedGF テクスチャ参照版） | **完了 (2026-04-27)** |
| IBL | Lazarov Env BRDF Approx | S06 | [[brdf_lazarov_env]] | `BRDF.ush:630` `EnvBRDFApproxLazarov` / `:642` `EnvBRDFApprox` | **完了 (2026-04-27)** |
| エリアライト | Linearly Transformed Cosines (LTC) | S08 | [[area_light_ltc]] | `RectLight.ush:334` | **完了 (2026-04-27)** |
...
| MSAA | Forward+ MSAA | （標準） | [[aa_msaa]] | （Forward Renderer） | **完了 (2026-04-27)** |
...
| DOF | Bokeh Convolution | S54 | [[post_dof]] | `DiaphragmDOF.cpp` | **完了 (2026-04-27)** |
| Motion Blur | Tile-based velocity scatter | S55 | [[post_motion_blur]] | `PostProcessMotionBlur.cpp` | **完了 (2026-04-27)** |
