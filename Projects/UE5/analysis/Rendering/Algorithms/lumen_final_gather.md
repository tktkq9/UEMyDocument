...
`OctahedronToUnitVector` / `UnitVectorToOctahedron` は `LumenScreenProbeCommon.ush:91` 等で双方向変換（関数定義は共通ヘッダ `Common.ush` 側）。理由: **キューブマップより歪みが均等**で、境界周辺の重複オーバーヘッドが少ない（border 1 ピクセルで足りる）。

> **補足（2026-04-30 監査差分）**: 旧記載の `:71` は UE5 現行ソースで `:91` に変更されている（+20 行）。エンジン更新で関数呼出し位置が下方移動したため修正。
