...
```
[テクスチャ生成フロー]
RHICreateTexture(Desc)                      ← グローバル関数（RHI.h インライン）
  └─ GDynamicRHI->RHICreateTexture(Desc)
       └─ FD3D12DynamicRHI::CreateTextureInternal()  D3D12Texture.cpp:622
            ├─ GetResourceDesc()            ← FRHITextureDesc → D3D12_RESOURCE_DESC 変換
            ├─ ID3D12Device::CreateCommittedResource()  or  PlacedResource()
            ├─ FD3D12Texture::CreateView()  ← RTV / DSV / SRV を生成
            └─ FD3D12Texture* を返す

[バッファ生成フロー]
RHICreateBuffer(RHICmdList, Desc, InitialState)
  └─ GDynamicRHI->RHICreateBuffer(...)
       └─ FD3D12DynamicRHI::RHICreateBufferInitializer()  D3D12Buffer.cpp:561
            ├─ BUF_Dynamic → FD3D12UploadHeapAllocator （Ring Buffer）
            ├─ BUF_Static  → FD3D12DefaultBufferAllocator
            └─ FD3D12Buffer* を返す

[PSO 取得フロー]
GetAndOrCreateGraphicsPipelineState(RHICmdList, Initializer, ...)
  └─ FD3D12PipelineStateCache::FindOrAdd()    ← メモリキャッシュを確認
       ├─ ヒット   → 既存 FD3D12PipelineState* を返す
       └─ ミス     → ID3D12Device::CreateGraphicsPipelineState()
                      PSOLibrary にもキャッシュ（ディスク保存）

[リソース解放フロー]
TRefCountPtr<FRHITexture> がスコープ外 → Release() → 参照カウント = 0
  └─ FRHIResource::MarkForDelete()
       └─ DeferredDeleteQueue に追加
            └─ ProcessDeferredDeletionQueue() (フェンス完了後)
                 └─ FRHIResource::DeleteResources()  ← 安全に破棄
```
...
| クラス / 関数 | ファイル:行 | 役割 |
|-------------|-----------|------|
| `RHICreateTexture()` | `RHI.h` インライン | GDynamicRHI 経由でテクスチャ生成 |
| `FD3D12DynamicRHI::CreateTextureInternal()` | `D3D12Texture.cpp:622` | D3D12 テクスチャ生成の実装本体 |
| `FD3D12DynamicRHI::RHICreateBufferInitializer()` | `D3D12Buffer.cpp:561` | D3D12 バッファ生成 |
| `FD3D12PipelineStateCache::FindOrAdd()` | `D3D12PipelineState.cpp` | PSO LRU キャッシュ管理 |
| `FRHIResource::MarkForDelete()` | `RHIResources.h` | 参照カウント=0 時に削除キューへ |
| `FD3D12Device::ProcessDeferredDeletionQueue()` | `D3D12Device.cpp` | フェンス完了後の安全な破棄 |
