...
```mermaid
flowchart TD
    A[InitialiseSubstrateFrameSceneData<br>MaterialTextureArray 等を RDG 上に確保]
    B[BasePass マテリアルシェーダー実行<br>各ピクセルにクロージャデータ書き込み]
    C[TopLayerTexture<br>最上位レイヤー情報書き込み]
    D[ClosureOffsetTexture<br>クロージャオフセット情報]

    A --> B
    B --> C & D
    C & D --> E[AddSubstrateMaterialClassificationPass<br>分類パスへ]
```
