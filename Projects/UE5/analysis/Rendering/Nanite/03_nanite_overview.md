...
graph TD
    subgraph Scene["① シーン管理"]
        ...
        SC[SceneExtension<br>Materials / Ownership]
        Prim --> SC
    end

    subgraph CullRaster["② カリング・ラスタライズ"]
        IC[Instance Culling<br>粗いフラスタム]
        CC[Cluster Culling<br>2パスオクルージョン]
        HW[HW Rasterizer<br>大三角形]
        SW[SW Rasterizer<br>微小三角形 Compute]
        VB[VisBuffer64<br>Depth + MaterialDepth]
        IC --> CC --> HW & SW --> VB
    end

    subgraph Materials["③ マテリアル・シェーディング"]
        Bin[Material Binning<br>ピクセル分類]
        Shade[Deferred Shading<br>ビン単位ディスパッチ]
        VB --> Bin --> Shade
    end
    ...
    subgraph Extensions["拡張"]
        RT[Ray Tracing<br>BLAS管理]
        SO[Stream Out<br>GPU→CPU]
