...
フェーズごとに適切なモデルを選択し、`/model` コマンドで切り替える。

| フェーズ | 推奨モデル | 理由 |
|---------|----------|------|
| フェーズ 0: ソースマップ | **Opus** | ソース構造の理解・グループ化の判断 |
| フェーズ 1: 概要 | **Opus** | アーキテクチャ全体の俯瞰・Mermaid 図設計 |
| フェーズ 2: Details | **Sonnet** | 構造確定後の記事執筆。量が多いので速度・コスト重視 |
| フェーズ 3: Reference | **Sonnet** | API 表の作成は構造的。Sonnet で十分 |
| フェーズ 4: Flow 追記 | **Opus** | コールチェーン追跡は複雑な推論が必要 |
| フェーズ 5: Reference 拡張 | **Sonnet** | 既存ファイルへの追記。パターンが決まっている |
| ue5-dive | **Opus** | アドホックな深掘りは推論力が必要 |
...
### サブエージェントによるモデル切り替えについて

Agent ツールの `model` パラメータで異なるモデルのサブエージェントを起動できるが、**通常のドキュメント作成では非推奨**:

- サブエージェントは毎回コールドスタート（システムプロンプト+ツール定義で ~20k トークンのオーバーヘッド）
- 会話コンテキストを引き継がないため、必要な情報を全てプロンプトに含める必要がある
- `/model` での手動切り替えはコンテキスト引き継ぎ済みなのでオーバーヘッドゼロ

**サブエージェントが有効なケース**: 完全に独立した調査を並列実行したいとき（例: 2つの異なるシステムのソースマップを同時作成）。ただしファイル書き込みの競合に注意。
